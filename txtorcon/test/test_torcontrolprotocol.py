...
class ProtocolTests(unittest.TestCase):
    ...
    def test_statemachine_broadcast_no_code(self):
        try:
            self.protocol.broadcastResponse("foo")
            self.fail()
    ...
    def test_statemachine_is_finish(self):
        self.assertTrue(not self.protocol.isFinishLine(''))
        self.assertTrue(self.protocol.isFinishLine('.'))
        self.assertTrue(self.protocol.isFinishLine('300 '))
        self.assertTrue(not self.protocol.isFinishLine('250-'))

    def test_statemachine_singleline(self):
        self.assertTrue(not self.protocol.isSingleLineResponse('foo'))

    def test_statemachine_continuation(self):
        try:
            self.protocol.code = 250
            self.protocol.isContinuationLine("123 ")
            self.fail()
        except RuntimeError, e:
